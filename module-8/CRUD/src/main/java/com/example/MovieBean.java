...
public class MovieBean {
    ...
    public List<Movie> getAllMovies() {
        ...
        try (Connection conn = DriverManager.getConnection(jdbcURL, jdbcUsername, jdbcPassword); // Establishing connection
    ...
    public Movie getMovieById(int id) {
        String sql = "SELECT * FROM steve_movies_data WHERE id = ?"; // SQL query to select a movie by ID
        try (Connection conn = DriverManager.getConnection(jdbcURL, jdbcUsername, jdbcPassword); // Establishing connection
             PreparedStatement stmt = conn.prepareStatement(sql)) { // Preparing SQL statement
